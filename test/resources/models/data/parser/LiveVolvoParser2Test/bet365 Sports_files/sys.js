...
b365.Ui.ToolTip.prototype = {show: function (f, i, c, b, h, j) {
  ...
  if (this._autoClosePopEl && this._autoCloseSrcEl && mf) {
    var b = $bSys.hitTest(this._autoCloseSrcEl, mf._mouseX, mf._mouseY), a = $bSys.hitTest(this._autoClosePopEl, mf._mouseX, mf._mouseY);
    if (!b && !a) {
      this.hide();
...
b365.Ui.MainForm.prototype = {pageInit: function () {
  ...
  $addHandler(window.document, "mousemove", Function.createDelegate(this, this.onMouseMove));
  Sys.Application.add_navigate(Function.createDelegate(this, this.restoreHistory))
}, onMouseMove: function (d) {
  var b = 0, c = 0, a = window.event || d;
  if (a && (a.pageX || a.pageY)) {
    b = a.pageX;
    c = a.pageY
  }
  else if (a && (a.clientX || a.clientY)) {
  ...
  !this._isAutoRefresh && b !== 29 && this.disconnectMatchLive();
...
b365.Ui.Coupon = function (a) {
  ...
  this.onLiveStreamClick = function () {
    var a = $get("embdStrm");
    if (lhs.getCurrentMode() == "INPLAY")a = $get("embdStrmIP");
    if (a)if ($(a).is(":visible"))$(a).hide();
    else if (parseInt(arguments[4])) {
      mf.req(arguments[4], arguments[1], arguments[2], arguments[3], 0);
      return false
    }
    return false
  };
  this.RefreshLiveStream = function () {
    ...
    if (d.length > 0)if (d.is(":visible")) {
      var c = $get("VideoContainer");
      if (c) {
        var a = c.getAttribute("data-rfs");
        if (!isNaN(parseInt(a))) {
          var g = (new Date).getTime() - mf._StrmLastUpdatedTime;
          if (Number(a) != 0 && !(Number(a) > g)) {
            var e = c.getAttribute("name"), b = e.split("#");
            mf.req(29, b[0], b[1], 0, ipoc._liveController.getMatchLiveAvailable(b[1])
              ? 1
              : 0)
          }
        }
...
function VideoStreamingController() {
  ...
  this.showLiveStreaming = function () {
    var b = "#embdStrmIP", a = "#livestreambtn", c = "#streamingWpr", d = "#streamContainer";
    $(b).show();
  ...
  this.hideLiveStreaming = function (c) {
    var a = "#embdStrmIP", b = "#livestreambtn";
    $(a).hide();
    $(a).children().remove();
    if (c)$(b).parent().show().addClass("visible");
...
(function (s, b, a, e) {
  ...
  var k = {}, n = true, r = e, q = {}, m = "#streamingWpr", c = "#mLiveCnt", l = "#embdStrmIP", j = "#livestreambtn", g = "#matchlivebtn", f = "#audiostreamingicon", h = "#streamContainer", i = "#audioCnt", d = "#holdingCnt", o = "#mlContainer";
  ...
  function p() {
    ...
    a(m).off("click.livecontroller").on("click.livecontroller", a.proxy(this.streamingOnClick, this))
  }

  p.prototype = {preLiveContentChange: function () {
    var f = a(c), g = a(l), j = a(i), d = a(h), m = a(o), e = a("#holdingCntChild"), b, k;
    if (f.is(":visible"))if (m.hasClass("dartsMlContainer"))b = d.height();
    else b = f.children("#mlContainer").height();
    else if (g.is(":visible"))b = g.children("#VideoContainer").height();
    else if (j.is(":visible"))b = j.children(".as-panel").height();
    else if (e.is(":visible"))b = e.height();
    else if (d)b = d.width() * .4;
    else b = 0;
    if (b < 10 && d)b = d.width() * .4;
    k = new b365.Ui.Storage.Accessor;
    k.setItem("lastMatchLiveHeight", b);
    this.toggleLiveStreaming(false, false);
    this.toggleMatchLive(false, false);
    this.toggleAudio(false, false)
  }, setParameters: function (a) {
    if (typeof a.classificationId !== "undefined")this._classificationId = a.classificationId;
    if (typeof a.fixtureId !== "undefined") {
      this._fixtureId = a.fixtureId;
      typeof a.hasMatchLive !== "undefined" && this.setMatchLiveAvailable(this._fixtureId, !!a.hasMatchLive)
    }
    if (typeof a.providerId !== "undefined") {
      this._providerId = a.providerId;
      if (this._providerId > 0)q[this._fixtureId] = this._providerId
    }
    if (typeof a.audioKey !== "undefined")this._audioKey = a.audioKey;
    if (typeof a.languageId !== "undefined")this._languageId = a.languageId;
    if (typeof a.matchLiveTopicId !== "undefined")this._matchLiveTopicId = a.matchLiveTopicId
  }, setParametersFromMarkup: function () {
    var b = a("#strmparameter"), c, d = null;
    if (b.length > 0) {
      if (b.attr("data-audiostreamingkey") != "") {
        var f = {stringKey: b.attr("data-audiostreamingkey")};
        d = new b365.Ui.AudioStreamingKey(f)
      }
    ...
    if (this.matchLiveFixtureState !== e)c.hasMatchLive = e;
    ...
    if (c)b = c.target
      ? c.target
      : c.srcElement;
    if (b && b.tagName === "A")if (!a(b).parent().hasClass("view--active"))if (b.id == "livestreambtn")this.showStreamingVideo(true);
    else if (b.id == "matchlivebtn")this.showMatchLive(true);
    else b.id == "audiostreamingicon" && this.showAudioStreaming(true)
  }, connectMatchLive: function () {
    var g = a("#mlContainer"), f, b, e, d, c;
    if (typeof MatchLiveGlobal != "undefined" && g.length > 0) {
      f = a("#cpn").attr("data-sportskey");
    ...
    if (typeof MatchLiveGlobal != "undefined")MatchLiveGlobal.onPageLoadClearMatchLive(true)
  }, liveContentLoaded: function (d, g) {
    d.stopPropagation();
    d.preventDefault();
    this.setParametersFromMarkup();
    this.setStreamingEventHandlers();
    if (!g && !ipoc.isInOverview()) {
      this.matchLiveFixtureState = e;
      var c = b.getElementById("MiddleScroller");
      c.scrollTop > 0 && a(c).scrollTop(0);
      var f = a(b.getElementById("mLiveCnt")), h = this.getMatchLiveAvailable(this._fixtureId)
        ? 1
        : 0;
      h == 1 && this.setHoldingContentHeight();
      f.hide();
    ...
    TickingClockController.start()
  }, getStreamingData: function (d, b) {
    if (!mf._isAutoRefresh && !mf._isManualRefresh) {
      var f = a(c);
      this.setHoldingContentHeight();
      this.clearStreamingContents("#embdStrmIP", false);
      this.clearStreamingContents("#matchLive", true);
      f.hide();
      var h = this._classificationId, g = this._fixtureId, e = this._providerId;
      if (typeof b === "undefined" || b === null)b = 0;
      if (e === 0)b = 1;
      if (b)d = this.getMatchLiveAvailable(this._fixtureId)
        ? 1
        : 0;
      if (e > 0 || d > 0) {
        if (d == 1)e = 0;
    ...
    return a
  }, setStreamingContent: function (u, e, i) {
    var s = this.isInplayMode()
      ? l
      : "#embdStrm";
    if (e) {
      var p = parseInt(e[1]), q = parseInt(e[2]), f = parseInt(e[3]), o = typeof e[4] !== "object" &&
        parseInt(e[4]) === 1, m = true, n = a(i), t = a("#strVideo", n), r = typeof e[4] === "object"
        ? e[4].autoRefresh
        : typeof e[5] === "object"
        ? e[5].autoRefresh
        : false, g = {};
      if (p > 0)g.classificationId = p;
      if (q > 0)g.fixtureId = q;
      if (f > 0)g.providerId = f;
      this.setParameters(g);
      if (o === true && !this.isInplayMode()) {
        a(h).hide();
        return false
      }
      var k = false;
      if (f > 0) {
        a(s)[0].innerHTML = n.outerHTML();
        ...
        var j = a(c);
        j[0].innerHTML = i;
        if (ipoc.isInOverview()) {
          a(d).hide();
          j.show()
        }
      }
      m && this.defaultLiveStreamingForFirstLoad(r);
      if (f > 0)if (i === "")this.getStreamingData(0, 1);
      else k === false && this.showStreamingVideo(0)
    }
    ...
    else this.showAudioStreamingButton() && a(i).length > 0 && !a(i).is(":visible") && this.showAudioStreaming(true);
    this.clearStreamingContents("#matchLive", true);
    a(c).hide();
    a(d).hide();
    this.toggleMatchLive(false, false)
    ...
    return this._audioKey != null && this._audioKey.keyString.length > 0
  }, setStreamingButtons: function (e, f, d) {
    ...
    var c = this.getMatchLiveAvailable(this._fixtureId)
      ? 1
      : 0, b = this.showLiveStreamingButton(), a = this.showAudioStreamingButton();
    this.toggleLiveStreaming(e, b);
    this.toggleMatchLive(f, c);
    this.toggleAudio(d, a);
    return false
  ...
  }, toggleLiveStreaming: function (e, c) {
    var b;
    if (this.isInplayMode())if (e) {
      _tabletEventAdapter.VideoStreamingController.showLiveStreaming();
      a(d).hide();
      this.isMatchLiveButtonEnabled() && a(g).parent().toggleClass("view--active");
      a(f).parent().hasClass("view--active") && a(f).parent().toggleClass("view--active")
    }
    else {
      b = a(l);
      b.is(":visible") && this.setHoldingContentHeight();
      _tabletEventAdapter.VideoStreamingController.hideLiveStreaming(c)
    }
    this.resetVisualButtonStyle()
  }, isMatchLiveButtonEnabled: function () {
    return a(g).parent().hasClass("view--active")
  }, toggleMatchLive: function (p, n) {
    var i, d, b, l, k;
    b = a(g);
    if (this.isInplayMode())if (p) {
      l = a(j);
      k = a(f);
      this.matchLiveFixtureState !== e && b.parent().show().addClass("visible");
      a(m).show();
      a(h).show();
      l.parent().hasClass("view--active") && l.parent().toggleClass("view--active");
      ...
      i = a(o);
      d = a(c);
      d.is(":visible") && this.setHoldingContentHeight();
      i.hasClass("dartsMlContainer") && i.removeClass("dartsMlContainer");
      if (this.matchLiveFixtureState !== e)if (n)b.parent().show().addClass("visible");
      else b.parent().hide().removeClass("visible");
      d.hide()
    }
    this.resetVisualButtonStyle()
  }, toggleAudio: function (o, n) {
    var e, b, l, k;
    e = a(i);
    b = a(f);
    if (this.isInplayMode())if (o) {
      l = a(g);
      k = a(j);
      e.removeClass("wait");
      e.show();
      ...
      e.is(":visible") && this.setHoldingContentHeight();
      e.hide();
    ...
    this.setStreamingButtons(false, true)
  }, showStreamingVideo: function (c, b) {
    if (!b)if (c) {
    ...
    var g = a(h), e = a(d), k = a("#sbRMN"), b;
    if (storage.getItem("lastMatchLiveHeight") > 0)b = storage.getItem("lastMatchLiveHeight");
    else {
      var i = new CouponKey(a("#cpn").attr("data-sportskey"));
      if (this.isMatchLive(i.classification) === true)if (g.width() > 0)b = g.width() * .4;
      else b = k.width() * .4;
      else b = 0
    }
    if (b > 0 && !(a(j).is(":visible") || a(f).is(":visible")) && e.is(":not(:visible)")) {
      var c;
      c = e.children("#holdingCntChild");
      c.css("height", b);
...
function MyBetsClass(y, u, l, b, d, c, t, r, v, s, e, k, j, m, n, p, q, o, a, h, g, f, i, x, w) {
  ...
  this.initialise = function (b) {
    ...
    if (b !== true)if (a.overlayOnDelegate)if (a.parentContext)a.overlayOnDelegate(a.parentContext);
